use crate::{TryMutex, Mutex, TryMutexSized, MutexSized};
use std::sync;
use std::sync::{MutexGuard, TryLockError};

impl<'a, T: ?Sized> TryMutex<'a> for sync::Mutex<T> where T: 'a{
    type Item = T;
...
impl<'a, T: ?Sized> Mutex<'a> for sync::Mutex<T> where T: 'a{
