use crate::{TryMutex, TryMutexSized, Mutex, MutexSized, TimeoutMutex, TimeoutMutexSized};
use parking_lot::MutexGuard;
use std::time::Duration;

impl<'a, T: ?Sized> TryMutex<'a> for parking_lot::Mutex<T> where T: 'a{
    type Item = T;
...
impl<'a, T: ?Sized> Mutex<'a> for parking_lot::Mutex<T> where T: 'a{
    fn lock(&'a self) -> Self::Guard {
        self.lock()
    }
}
impl<'a, T> MutexSized<'a> for parking_lot::Mutex<T> where T: 'a{}
impl<'a, T: ?Sized> TimeoutMutex<'a> for parking_lot::Mutex<T> where T: 'a{
